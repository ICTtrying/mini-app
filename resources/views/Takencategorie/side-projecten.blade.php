    ...
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        ...
        <div class="grid auto-rows-min gap-4 md:grid-cols-3">
            <div id="dashboardTaskViewContainer" class="bg-zinc-300 dark:bg-zinc-600 lg:col-span-2 md:col-span-2 col-span-1">
                <h2>
                    overzicht over uw side-project taken:
                </h2>
                    @forelse ($taken as $taak)
                        <form action="{{ route('checkTaakSideProjecten') }}" method="POST" class="taak-form">
                            @csrf
                    ...
                    @forelse($taken as $taak)
                        @if ($taak->deadline && \Carbon\Carbon::parse($taak->deadline)->isPast() && $taak->status !== 'klaar')
                            <li class="mb-2 border-2 p-2 rounded-2xl">
                                <form action="{{ route('checkTaakSideProjecten') }}" method="POST" style="display: flex; align-items: center; gap: 10px;">
                                    @csrf

                                    <input type="hidden" name="taak_id" value="{{ $taak->id }}">

                                    <input type="checkbox" class="checkbox"
                                        style="transform: scale(1.5); margin-right: 0;"
                                        onchange="this.form.submit()" {{ $taak->status === 'klaar' ? 'checked' : '' }}>

                                    <span class="font-semibold">{{ $taak->titel }}</span>
                                    <span class="text-sm text-red-600">(Deadline: {{ $taak->deadline }})</span>

                                </form>
                                <small>{{ $taak->omschrijving }}</small>
                                </form>
                            </li>
                        @endif
                    @empty
                        <li class="text-red-400">Geen taken gevonden.</li>
                    @endforelse
                </ul>
            </div>
        </div>
